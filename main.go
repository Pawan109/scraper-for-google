...
func randomUserAgent() string {
	...
	return userAgents[randNum] //this is how you choose a random userAgent
}
...
func googleResultParsing(response *http.Response, rank int) ([]SearchResult, error) {
	doc, err := goquery.NewDocumentFromResponse(response) //jo bhi result i.e response hai => uska doc bana liya

	if err != nil {
		return nil, err
	}

	results := []SearchResult{} //results naam ki slice banayi? jisko SearchResult se initialise kiya ??
	sel := doc.Find("div.g")    //div.g find krna hai in the doc but kyu?
	rank++

	//html se hume rank, link , title, desc extract krna hai (scrape)
	for i := range sel.Nodes {
		item := sel.Eq(i)               //?
		linkTag := item.Find("a")       //link tag hota hai <a> mein
		link, _ := linkTag.Attr("href") //aur link hoti hai href mein
		titleTag := item.Find("h3.r")
		descTag := item.Find("span.st")
		desc := descTag.Text()
		title := titleTag.Text()
		link = strings.Trim(link, " ") //link se ab blank spaces hata do

		//if link is not empty  & link isn't a NULL link -> # mtlb NULL link hota hai
...
func main() {
	res, err := GoogleScrape("Pawan Jumani", "com", "en", nil, 1, 30, 10)
	if err == nil {
		for _, res := range res {
			fmt.Println(res)
		}
